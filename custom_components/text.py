...
class MegaDDisplayEntity(CoordinatorEntity, TextEntity):
    ...
    def __init__(
        ...
        # ✅ КРИТИЧЕСКОЕ ИСПРАВЛЕНИЕ: каждая сущность дисплея получает УНИКАЛЬНЫЙ device_info
        self._attr_device_info = coordinator.entity_device_info(
            port.conf.name,                            # Имя сущности
            f"MegaD-{self._megad.id} Display",         # Модель устройства
            entity_type="display",                     # Тип сущности
            port_id=port.conf.id                       # ID порта
        )
