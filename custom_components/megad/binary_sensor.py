...
class BinarySensorMegaD(CoordinatorEntity, BinarySensorEntity):
    ...
    async def async_added_to_hass(self):
        ...
        # ✅ СРАЗУ ОТМЕЧАЕМ АКТИВНОСТЬ ПРИ СОЗДАНИИ
        if hasattr(self.coordinator, 'watchdog') and self.coordinator.watchdog:
            self.coordinator.watchdog.mark_feedback_event({
                "type": "binary_sensor_created",
    ...
    def _handle_coordinator_update(self) -> None:
        ...
        if self._last_state is not None and self._last_state != current_state:
            ...
            # ✅ ВАЖНО: ОТМЕЧАЕМ ПОЛУЧЕНИЕ ДАННЫХ ДЛЯ WATCHDOG
            # При изменении состояния бинарного сенсора отмечаем активность
            if hasattr(self.coordinator, 'watchdog') and self.coordinator.watchdog:
                self.coordinator.watchdog.mark_data_received()
                
                # ✅ ТАКЖЕ ОТМЕЧАЕМ КАК СОБЫТИЕ ОБРАТНОЙ СВЯЗИ
                # (бинарные сенсоры часто меняют состояние при взаимодействии)
                self.coordinator.watchdog.mark_feedback_event({
                    "type": "binary_sensor_update",
                    "entity_id": self.entity_id,
                    "port_id": self._port.conf.id,
                ...
                _LOGGER.debug(f"Watchdog отмечен для binary sensor {self.entity_id}")
    ...
    def extra_state_attributes(self):
        ...
        # Добавляем информацию о watchdog
        if hasattr(self.coordinator, 'watchdog') and self.coordinator.watchdog:
            if hasattr(self.coordinator.watchdog, '_last_incoming_data'):
                last_data = self.coordinator.watchdog._last_incoming_data
                if last_data:
                    attributes["watchdog_last_data"] = last_data.isoformat()
...
class BinarySensorExtraMegaD(CoordinatorEntity, BinarySensorEntity):
    ...
    async def async_added_to_hass(self):
        ...
        if hasattr(self.coordinator, 'watchdog') and self.coordinator.watchdog:
            self.coordinator.watchdog.mark_feedback_event({
                "type": "extra_binary_sensor_created",
                "entity_id": self.entity_id,
    ...
    def _handle_coordinator_update(self) -> None:
        ...
        if self._last_state is not None and self._last_state != current_state:
            ...
            # ✅ ВАЖНО: ОТМЕЧАЕМ ПОЛУЧЕНИЕ ДАННЫХ ДЛЯ WATCHDOG
            # При изменении состояния дополнительного бинарного сенсора отмечаем активность
            if hasattr(self.coordinator, 'watchdog') and self.coordinator.watchdog:
                self.coordinator.watchdog.mark_data_received()
                
                # ✅ ТАКЖЕ ОТМЕЧАЕМ КАК СОБЫТИЕ ОБРАТНОЙ СВЯЗИ
                self.coordinator.watchdog.mark_feedback_event({
                    "type": "extra_binary_sensor_update",
                    "entity_id": self.entity_id,
                    "base_port_id": self._port.conf.id,
                    "extra_port_id": self._config_extra_port.id,
                    "old_state": self._last_state,
                    "new_state": current_state,
                    "sensor_type": "extra_binary"
                })
                
                _LOGGER.debug(f"Watchdog отмечен для extra binary sensor {self.entity_id}")
